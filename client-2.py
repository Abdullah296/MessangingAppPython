...
class Client
    ...
    # In Request type
    #       SignUp Request
    #           request format ruppassword
    #           response format resupunique_Id
    #       SignIn Request
    #           request format rinIDpassword
    #           response format resin'True''False'
    #       Information Request (for current status onlineoffline)
    #           request format rinfoClientIDClientID ... or rinfoGroupID
    #           response format resinfoIDStatusIDStatus ...
    #       Leave Group Request
    #           request formate rlgGroup_ID
    #           responce formate reslgTrueFalseGroup_ID
    # In Command Type
    #       Create Group
    #           request formatccgGroupNameIDIDID
    #           response formatrescgGroupIDGroupName (g+ID mean starts with g always)
    #       Remove from Group
    #           request formatcrfgMember's_IDGroup_ID
    #           response formatresrfg'True''False'message from server
    #       Add to Group
    #           request formatcatgMember's_IDGroup_ID
    #           response formatresatg'True''False'message from server
    #       Change Admin
    #           request formatccaNew_Admin_IDGroup_ID
    #           response formatresca'True''False'New_Admin
    # In Messsage Type
    #       Message to a single Client
    #           request formatmOtherClient's_IDmessage
    #           response format mSender_IDmessage (when ever you will recieve a messaage)
    #       Message in a Group
    #           request formatmGroup_IDmessage
    #           response format mGroup_IDSender_IDmessage
    # request from server type
    #       Group Joining Request
    #           request from server  reqgjrGroup_IDGroup_Name
    #           responce to server   resgjrGIDResponce
    #
    # Responce types
    ...
    def Contacts(self)
        ...
        #       How it will do
        #            build the request formate (rinfoidid ...)
        #            create a request to server(with proper format)
        #            take response from the server (Client_IDstatusClient_IDstatus ...)
        #            decodes it by spliting it with '' (['Client_IDstatus','Client_IDstatus'])
    ...
    def GroupMembers(self)
        ...
        #       How it will do
        #            builds the request formate (rinfoGroup_ID)
        #            make a request to server
        #            takes response from server (Member's_IDstatusMember's_IDstatus ...)
        #            decodes and print it
        #       Other
        if self.MyStatus is True
            msg = rinfo
            # rinfo'group ID' format
            # printing joined groups
            for gID, Name in self.MyGroups.items()
                print(fGroup ID {gID} Group Name {Name})
    ...
    def SignUp(self)   # will do sign Up
        ...
        #            build request foramte (ruppassword)
    ...
    def SignIn(self)   # will do sign in
        ...
        #            build request formate (rinIDpassword)
        ...
        temp = self.MyInput(Enter Your Password )
        temp = rin+str(self.MyId)++str(temp)
        self.Socket.sendall(temp.encode('UTF-8'))
        msg = self.Socket.recv(1024).decode('UTF-8')
        msg = msg.split()
        if msg[1] == 'in'
            if msg[2] == 'True'
                print(nSign in successful)
                self.MyStatus = True
    ...
    def CreateGroup(self)
        ...
        #            build the request formate (ccgmember's_IDmember's_ID ...)
    ...
    def RemoveFromGroup(self)
        ...
        #            build request formate (crfgMember's_IDGroup_ID)
        #            make request to server
        #            take response from the server ('Admin changed''You are not the admin of this group')
        #       Other
    ...
    def AddToGroup(self)
        ...
        #            build request formate (catgNew_Member's_IDGroup_ID)
        #            make request to server
    ...
    def NotificationHandler(self)
        ...
        if len(self.Notifications['Requests']) == 0
            ...
            print(fGot Requests ({len(self.Notifications['Requests'])}))
        if len(self.Notifications['Messages']) == 0
            print(No Message)
        else
            print(fGot Messages ({len(self.Notifications['Messages'])}))
    ...
    def Receive(self)
        while True
            ...
            if msg[0] == 'res'     # it's a response from a server
                    ...
                    if msg[2][0] == 'S'
                        ...
                        for EachInfo in msg[2-1]
                            ...
                            if EachInfo[0] in self.MyContacts.keys()
                                print(fName {self.MyContacts[EachInfo[0]]}, fID {EachInfo[0]},
                                      fstatus {EachInfo[1]},
                                      sep='     ')
                            else
                                print(fName Not Saved, fID {EachInfo[0]}, fstatus {EachInfo[1]}, sep='     ')
                if msg[1] == 'cg'
                ...
                if msg[1] == 'ca'
                    if msg[2] == 'True'
                        ...
                        if msg[3] in self.MyContacts.keys()
                            print(fNew Group Admin is {self.MyContacts[msg[3]]})
                        else
                            print(fNew Group Admin is {msg[3]})
                ...
                if msg[1] == 'lg'
                    if msg[2] == 'True'
                        print(fYou left Group{msg[3]})
                    elif msg[2] == 'False'
                        print(fYou can't left Group{msg[3]})
            elif msg[0] == 'm'     # a message
                if msg[1][0] == 'g'        # a group ID
                    if msg[2] in self.MyContacts.keys()
                        tmsg = fIn Group -- {self.MyGroups[msg[1]]} - {self.MyContacts[msg[2]]} -- {msg[3]}
                        self.Notifications['Messages'].append(tmsg)
                    else
                        tmsg = fIn Group -- {self.MyGroups[msg[1]]} - {msg[2]} -- {msg[3]}
                        self.Notifications['Messages'].append(tmsg)
                else       # a user ID
    ...
    def Decoder(self)
        #       What it will do
        #           it will ask user for to enter choise or what heshe wants to do
        #       How it will do
